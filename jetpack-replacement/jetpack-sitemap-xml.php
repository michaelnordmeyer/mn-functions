...
function jetpack_load_xsl( $type = '' ) {
	...
	include_once 'jetpack-sitemap-xsl.php';
	...
	foreach ( $posts as $post ) {
		...
		if ( ! post_password_required( $post->ID ) ) {
			...
			foreach ( $methods as $method => $value ) {
				$methods[ $method ] = true;
/*mn				$images_collected = Jetpack_PostImages::get_images( $post->ID, $methods );
...
//mn			$post_thumbnail = Jetpack_PostImages::get_image( $post->ID );
